...
public class PedestrianObject : MonoBehaviour 
{
    ...
    {
        /*Vector3 relativeVector = transform.InverseTransformPoint(m_currentNode.transform.position);
    ...
    void Update()
    {
        ...
        {
            if (gameObject.GetComponent<Rigidbody>())//rigidbody)                                                                                              // if we have a rigidbody, use the following code to move us
            {
            ...
            else                                                                                                                // no rigidbody then use the following code to move us
            {
                ...
                if (gameObject.GetComponent<Collider>())//collider)                                                                                                    // it generally is a bad idea to move something with a collider, so we should tell someone about it if this is happening. See Unity Docs for more info: http://docs.unity3d.com/ScriptReference/Collider.html
